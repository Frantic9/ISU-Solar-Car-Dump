...
#Homemade boards
    {"can_id": 0x728, "can_mask": 0xffff},#info from pedal [unused, GPIO, A1, A2, unused, A3, A4, unused]
...
def errReset():
    ...
    for i in errorCode:
        if errorCode[i]>1:
            msg = msg + " " + str(i)
    power=False
    bus.send(bus.Message(arbitration_id=0x70a, data=[0x00,0x44]))
    bus.send(bus.Message(arbitration_id=0x712, data=[0x00,0x44]))
...
def logToFile():
    global log, chgCurrentAbsMax, popCells, charge, discharge, precharged, brk, speed, motV, mpg, mcT, trackerBusCurrent, errorCode, busI, battV, motRpm
    while True:
        if log:
            f.write("%f, %.3f, %.3f, %.3f, %.3f, %.3f \r\n" % (time.time(),speed,motV,busI,battV,sum(trackerBusCurrent)))
            time.sleep(1)

        
def recCan():
    global power, chgCurrentLimit, chgCurrentAbsMax, popCells, charge, discharge, precharged, brk, speed, motV, mpg, mcT, trackerBusCurrent, errorCode, busI, battV, motRpm
    while True:
        if power:
            f.write("%f, %.3f, %.3f, %.3f, %.3f, %.3f \r\n" % (time.time(),speed,motV,busI,battV,sum(trackerBusCurrent)))
        try:
        ...
        try:
            if id == 0x36:
               ...
               label5.config(text="Array I:\n%3.1f" % sum(trackerBusCurrent))
                ...
                auxPack = (((packet.data[2]*1.35)+680)/1024)*15
                button8.config(text="Aux V:\n%2.2f" % auxPack)
       #elif id == 0x710:
       #    battGPIO
        except:
            pass
...
def sendCan():
        global reg, count, aCh, gr, cru, array, bmsLt, maxI, chgCurrentAbsMax, chgCurrentLimit, brk, power, lftTur, ritTur, lght, stopButton, discharge, errorCode, charge, precharged
        while True:
          try:
            label9.config(text="Throttle: "+str(round(100*aCh[0]/255,1))+"%")
            if reg==1:
                button14.config(text= "Regen:\n "+str(round(chgCurrentAbsMax*aCh[1]/255,2))+"A")
            busCurrent = struct.pack('f', 100.0)
            motorCurrent = struct.pack('f', 0.0)
            speed = struct.pack('f', 0)
            #horn and parking brake
            bus.send(can.Message(arbitration_id=0x720, data=[horn.is_pressed,0x55, pBrake.is_pressed], extended_id=False))
            time.sleep(0.005)
            ...
            #    time.sleep(0.010)
            #    count=count+1
            #elif errorCode[0]==1 and count>0:
            #    print("s")
            #    time.sleep(0.010)
            #else:
            msg = can.Message(arbitration_id=0x70a, data=[(((array)<<7)|((sum(errorCode)>0)<<6)|((brk)<<5)|((ritTur)<<4)|((lftTur)<<3)|((lght)<<2)),0x55],extended_id=False)
            ...
            #    count=0
            #battery board msg
            boul = (discharge > 0) and power and (charge > 0)
...
#r2push = Button(27)
#r2pull = Button(23)
horn = butt(22) #use horn.is_pressed in the cansend funtion
#-----------------------------------------


button1 = Button(root, height=3, width=9, text = "Power On",font=("Ubuntu",18), command=chgPower)
button2 = Button(root, height=3, width=9, font=("Ubuntu",18),text = "RESET ERRORS", command=errReset)
...
button8 = Label(root, height=3, width=9, text = "Aux V:\n0.0",font=("Ubuntu",18))
...
label4 = Label(root, height=3, width=12, text = "kWh:",font=("Ubuntu",18), wraplength=120)
