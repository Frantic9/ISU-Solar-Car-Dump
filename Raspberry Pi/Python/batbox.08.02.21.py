...
def recCan():
    ...
    global midRecTime, whlRecTime, horn, gearInput, highCellVStack
    while True:
        try:
            packet = bus.recv(1)
...
def checkVitals():
    global errorCode, lowCell, highCell, battI, battTempH, battTempL, discharge, maxI, highCellVStack
    #print( 'check vitals')
    #print(highCell)
    #print(sum(highCellVStack)/len(highCellVStack))
    if lowCell<2.505:
        print('lowV')
        errorCode=49
        return False
    elif (sum(highCellVStack)/len(highCellVStack))>4.195:
        print('highV: %f' % highCell)
        errorCode=50
        return False
    elif battI>maxI:
        print('highI')
